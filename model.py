...
from transformers import ElectraForSequenceClassification, ElectraModel, BertModel, BertForSequenceClassification, AutoModel
...
class primary_encoder(nn.Module):
    ...
    def __init__(self, batch_size, hidden_size, label_size, encoder_type="electra"):
        ...
        self.pooler_fc = nn.Linear(hidden_size, hidden_size)  # 全连接层
        self.pooler_dropout = nn.Dropout(0.1)
        self.label = nn.Linear(hidden_size, label_size)  # 线性层，输出分类的logits
...
class weighting_network(nn.Module):
    ...
    def __init__(self, batch_size, hidden_size, label_size, encoder_type="electra"):
        ...
        if encoder_type == "electra":
            options_name = "google/electra-base-discriminator"
            self.encoder_supcon_2 = ElectraForSequenceClassification.from_pretrained(options_name, num_labels=label_size)
            # to make it faster
